...
export const FileUpload: React.FC<FileUploadProps> = ({ onUploadSuccess }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected) {
      ...
      setTimeout(() => {
        setStatus('success');
        addToast('success', `Datei "${selected.name}" erfolgreich hochgeladen.`);
  ...
  return (
    <div className="bg-panel border border-line rounded-2xl p-5 space-y-4 max-w-sm">
      <div className="flex items-center space-x-2 border-b border-line pb-3">
        ...
        <h3 className="font-bold text-xs uppercase text-muted tracking-wider">Datei-Upload (Assignments)</h3>
      </div>
      ...
      <div className="relative border-2 border-dashed border-line rounded-xl p-6 text-center hover:border-cyan/50 transition-colors">
        ...
        {status === 'idle' && (
          <div className="space-y-2">
            ...
            <div>
              <p className="text-xs font-semibold text-text">Klicke zum Auswählen oder Drag & Drop</p>
              <p className="text-[10px] text-muted mt-1">PDF, ZIP, IPYNB bis zu 50 MB</p>
            </div>
          </div>
        )}
        ...
        {status === 'scanning' && (
          <div className="space-y-2 py-2">
            ...
            <p className="text-xs font-bold text-cyan">Virenprüfung & Upload läuft...</p>
          </div>
        )}
        ...
        {status === 'success' && file && (
          <div className="space-y-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-green font-semibold mt-1">Sicher hochgeladen & virengeprüft</p>
            </div>
          </div>
        )}
      </div>
